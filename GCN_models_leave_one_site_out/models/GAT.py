...
class NestedGAT(torch.nn.Module):
    def __init__(self, input_dim, hidden, use_z=False, use_rd=False, num_layers=2, num_classes=2, hidden_linear=64, n_node=90):
        ...
        self.lin1 = torch.nn.Linear(n_node * num_layers * hidden, hidden_linear)
...
class GAT(torch.nn.Module):
    ...
    def __init__(self, input_dim, hidden, *args, num_classes=2, num_layers=2, hidden_linear=64, n_node=90, **kwargs):
    ...
    def forward(self, data, return_features=False):
        ...
        hidden_features = F.relu(self.lin1(x))
        classifier_features = F.dropout(hidden_features, p=0.5, training=self.training)
        logits = self.lin2(classifier_features)
