...
from functions_SGCN import run_leave_one_site_cv
import argparse
...
def model_factory(model, net, n_node, task):
    if model == 'GAT':
        ...
        Net = GCN(input_dim=load_dim(net, task=task), hidden=128, n_node=n_node)
        ...
        Net = SGCN_GCN(input_dim=load_dim(net, task=task), hidden=128)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-net', default='BrainANet_SwinVIT',
    ...
    parser.add_argument('-g', default=['CN', 'AD'], nargs='+', help='Group', type=parse_str_or_int)  # ['CN', 'AD']
    parser.add_argument('-task', default='Dementia', help='Task(Dementia,ASD,MDD,ASD_multimodal,MDD_multimodal)')
    ...
    all_predictions, site_metrics = (
        run_leave_one_site_cv(
            ...
            output_dir=f"./LOSO_results8/{args.net}.{args.model}.{args.task}.{args.g[0]}_{args.g[1]}.{args.seed}",
            n_node=90,
            num_epochs=100,
            batch_size=8,
            learning_rate=1e-4,
