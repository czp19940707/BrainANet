...
def show(args, fold_name, stage, Phase):
    ...
    plt.figure(figsize=(figure_width * 1.25, figure_width), dpi=300)  # 设置DPI为300
    plt.plot([0, 1], [0, 1], lw=line_width, linestyle='--', color='red')
    plt.xlim([-0.05, 1.05])
    plt.ylim([-0.05, 1.05])
    ...
    for nums, (method, legend_) in enumerate(zip(method_list, legend_list)):
        ...
        plt.plot(fpr, tpr,
                 label=legend_ + ': {}'.format(str(roc_auc)[:6]), lw=line_width, alpha=.8,
    ...
    plt.legend(loc='lower right', ncol=1, fontsize=fontsize_legend)
    ...
    fig, ax = plt.subplots(figsize=(figure_width * 2, figure_width), dpi=300)
    bar_width = (figure_width) / (len(indexes) * 8)
    ...
    # 计算legend的行数
    num_legend = len(indexes)  # 图例项的数量
    ncol = min(3, num_legend)  # 设置列数为3，如果图例项少于3个则保持不变
    ...
    ax.tick_params(axis='y', labelsize=fontsize_label * 1.2)
    ax.tick_params(axis='x', labelsize=fontsize_label * 1.2)
    # ax.grid(True)
    ax.legend(loc='best', ncol=ncol, fontsize=fontsize_legend * 1.2)
    ...
    fig, ax = plt.subplots(figsize=(figure_width, figure_width), subplot_kw=dict(polar=True))
    ...
    for nums, index_ in enumerate(frame_save.index):
        ...
        ax.plot(angles, values, label=category, linewidth=line_width, color=color_selected[nums], marker='o', markersize=line_width * 3)
    ...
    ax.set_xticklabels(metrics, fontsize=fontsize_label, fontweight='bold', color='black')
    ...
    ax.set_yticks(y_)
    ax.set_yticklabels(y_str, color="gray", size=fontsize_label)
    ax.set_ylim(np.min(y_), np.max(y_))

    ax.set_title(args.s, size=12, pad=20, weight='bold')

    # 计算legend的行数
    num_legend = len(method_list)  # 图例项的数量
    ncol = min(3, num_legend)  # 设置列数为3，如果图例项少于3个则保持不变

    # 调整图例位置并设置列数
    ax.legend(loc='lower center', bbox_to_anchor=(0.5, -0.3), ncol=ncol, fontsize=fontsize_legend * 1.2, frameon=False)

    plt.subplots_adjust(top=0.9, bottom=0.3)  # 调整边距以适应图例
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', default='results_MDD_model_comparisons', type=str, help='Task name')
    parser.add_argument('-s', default='comparison of different models on NC vs. MDD', type=str,
                        help='想要对比什么，起个名字')
    parser.add_argument('-stage', default='test', type=str, help='train or test')
    parser.add_argument('-Phase', default=None, type=str, help='dataset name')
